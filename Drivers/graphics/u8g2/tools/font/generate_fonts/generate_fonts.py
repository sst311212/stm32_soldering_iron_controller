...
if __name__ == "__main__":
    ...
    try:
        ...
        uni_mapset = "32-126,160-255,268-271,282-283,286,287,304,305,327,328,344,345,350-353,356,357,366,367,381,382,937,1040-1103"
        chi_mapset = "32-126,160-255,937,5000-65535"
        exec_command([r"..\bdfconv\bdfconv.exe", "-v", "-b", "0", "-f", "1", "-m", uni_mapset, "-o", r"temp\font_menu_uni.c", "-n", "u8g2_font_menu", r"temp\font_menu_all.bdf"])
        exec_command([r"..\bdfconv\bdfconv.exe", "-v", "-b", "0", "-f", "1", "-m", chi_mapset, "-o", r"temp\font_menu_chi.c", "-n", "u8g2_font_menu", r"temp\font_menu_all.bdf"])
        exec_command([r"..\bdfconv\bdfconv.exe", "-v", "-b", "0", "-f", "1", "-m", uni_mapset, "-o", r"temp\font_smol_uni.c", "-n", "u8g2_font_small", r"temp\font_smol_all.bdf"])
        exec_command([r"..\bdfconv\bdfconv.exe", "-v", "-b", "0", "-f", "1", "-m", chi_mapset, "-o", r"temp\font_smol_chi.c", "-n", "u8g2_font_small", r"temp\font_smol_all.bdf"])
        ...
        replaceFileString(r"temp\font_menu_uni.c", "\\21\\", "\\17\\")
        replaceFileString(r"temp\font_menu_chi.c", "\\20\\", "\\17\\")
        replaceFileString(r"temp\font_smol_uni.c", "\\16\\", "\\12\\")
        replaceFileString(r"temp\font_smol_chi.c", "\\16\\", "\\12\\")

        # Combine all font sources into one file
        data = open(r"temp\font_iron_temp.c").read()
        data += "#ifndef CHINESE\n"
        data += open(r"temp\font_menu_uni.c").read()
        data += open(r"temp\font_smol_uni.c").read()
        data += "#else\n"
        data += open(r"temp\font_menu_chi.c").read()
        data += open(r"temp\font_smol_chi.c").read()
        data += "#endif\n"
        open("u8g2_aio.c", "w").write(data)
